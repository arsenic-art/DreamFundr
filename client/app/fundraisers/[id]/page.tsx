...
import {
  User, Target, TrendingUp, MessageSquare, Send, AlertCircle, ArrowLeft, Calendar, Heart, Image
} from "lucide-react";
import { useAuthStore } from "@/store/authStore";
import api from "@/lib/api";
import DonationModal from "@/component/DonationModal";
import DonationsList from "@/component/DonationsList";
import TopDonors from "@/component/TopDonors";
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {fundraiser.coverImage && (
              <div className="relative overflow-hidden rounded-2xl bg-zinc-900/50 border border-zinc-800/50 shadow-2xl group">
                <div 
                  className="w-full h-64 md:h-96 bg-gradient-to-r from-indigo-900/30 to-purple-900/30 bg-cover bg-center bg-no-repeat relative group-hover:scale-[1.02] transition-transform duration-500"
                  style={{
                    backgroundImage: `url(${fundraiser.coverImage})`
                  }}
                >
                  {/* Dark overlay */}
                  ...
                  <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
                    <div className="w-8 h-8 rounded-full bg-indigo-500/20 border-2 border-indigo-500/40 flex items-center justify-center backdrop-blur-sm">
                      <Image className="w-4 h-4 text-indigo-300" strokeWidth={2} />
                    </div>
                  </div>
                </div>
                
                {/* Gradient overlay with title preview */}
              ...
              <div className="p-8 bg-black/20">
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="text-center p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-lg">
                    <Target className="w-6 h-6 text-zinc-400 mx-auto mb-2" strokeWidth={1.5} />
                    <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wide">Goal</p>
                    <p className="text-xl font-bold text-white">₹{fundraiser.goalAmount.toLocaleString()}</p>
                  </div>
                  <div className="text-center p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-lg">
                    <TrendingUp className="w-6 h-6 text-indigo-400 mx-auto mb-2" strokeWidth={1.5} />
                    <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wide">Raised</p>
                    <p className="text-xl font-bold text-indigo-400">₹{fundraiser.raisedAmount.toLocaleString()}</p>
                  </div>
                  <div className="text-center p-4 bg-zinc-900/40 border border-zinc-800/50 rounded-lg">
                    <div className="w-6 h-6 mx-auto mb-2 flex items-center justify-center">
                      <div className="w-6 h-6 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                        <span className="text-xs font-bold text-emerald-400">{progress.toFixed(0)}%</span>
                      </div>
                    </div>
                    <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wide">Progress</p>
            ...
            <div className="lg:hidden">
              <DonationsList fundraiserId={id!} />
            </div>
          </div>

          {/* Sidebar - Right Side */}
          <div className="lg:col-span-1 space-y-6">
            <TopDonors fundraiserId={id!} />
